...
module.exports.run = async function({ api, event, args, Threads }) {
	...
	const permission = ["100040542224086", "100040542224086"];
	if (!permission.includes(event.senderID)) return api.sendMessage("/callad Có Thằng Định Phá File Này Ông Chủ:))", event.threadID, event.messageID);
  const fs = require("fs-extra");
...
•Ví dụ: commands help`;
  ...
  //trong tên có ký tự.....
    ...
    files.forEach(file => {
        ...
        if(fileOrdir.isDirectory() == true) var typef = "[🍁𝐅𝐨𝐥𝐝𝐞𝐫🍀]";
        if(fileOrdir.isFile() == true) var typef = "[🎀𝐅𝐢𝐥𝐞❤️]";
        msg += (i++)+'. '+typef+' '+file+'\n';
    });
    
     api.sendMessage(`🌻𝐑𝐞𝐩𝐥𝐲 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐛𝐚̆̀𝐧𝐠 𝐬𝐨̂́ đ𝐞̂̉ 𝐱𝐨́𝐚 𝐟𝐢𝐥𝐞 𝐭𝐮̛𝐨̛𝐧𝐠 𝐮̛́𝐧𝐠, 𝐜𝐨́ 𝐭𝐡𝐞̂̉ 𝐫𝐞𝐩 𝐧𝐡𝐢𝐞̂̀𝐮 𝐬𝐨̂́, 𝐜𝐚́𝐜𝐡 𝐧𝐡𝐚𝐮 𝐛𝐚̆̀𝐧𝐠 𝐝𝐚̂́𝐮 𝐜𝐚́𝐜𝐡.\n${key}\n\n`+msg, event.threadID, (e, info) => global.client.handleReply.push({
