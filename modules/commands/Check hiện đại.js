...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n[🖤min🖤] => check ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n[🖤min🖤] => check del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n[🖤min🖤] => check onl => Xem thời gian hoạt động bot onl \n--------\n[🖤min🖤] => check tt => Check độ tương tác của bạn\n--------\n[🖤min🖤] => check box => Lọc nhóm dưới 4 thành viên\n--------\n[🖤min🖤] => check all => Check tất cả độ tương tác các thành viên trong box\n--------\n[🖤min🖤] => check covid => Xem thông tin covid\n--------\n[🖤min🖤] => check mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n[🖤min🖤] => check nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
    ...
    if (!find) return api.sendMessage(`[🖤min🖤] => Bạn và bot cần là quản trị viên!`,event.threadID);
      let storage = [];
    ...
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[🖤min🖤] => Bạn và bot cần là quản trị viên!`,event.threadID);
    if (!args[1]) return api.sendMessage(`[🖤min🖤] => HDSD: check del => số tin nhắn cần lọc `,event.threadID);
      let storage = [];
  ...
  return api.sendMessage(
	...
	let seconds = Math.floor(time % 60);
      const timeStart = Date.now();
       return api.sendMessage('[🖤min🖤] => Đang kiểm tra kết nối vui lòng chờ... !', event.threadID, (err, info) => {
    setTimeout(() => {
      api.sendMessage(`[🖤min🖤] => Ping: ${(Date.now() - timeStart)}ms \n[🖤min🖤] => Thời gian bot hoạt động: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
