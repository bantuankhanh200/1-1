...
module.exports.onLoad = function() {
    ...
    const path = pathA.join(__dirname, 'cache', 'v2.json');
    if (!existsSync(path)) {
        const obj = []
...
module.exports.handleEvent = async({ event, Users }) => {
    ...
    const fs = require("fs");
    const pathA = require('path');
    const thread = require('./cache/v2.json');
    const path = pathA.join(__dirname, 'cache', 'v2.json');
  ...
  const t = Date.parse("February 1, 2022 00:00:00") - Date.parse(new Date());
    ...
    if (args[0] == "all") {
        ...
        var page = 1;
            ...
            page < -1 ? page = 1 : "";
            var msg = "🌺 check tương tác theo tháng🌺 1 tháng admin sẽ xoá data 1 lần 💖";
