...
module.exports.handleReply = async function ({ api, event, handleReply }) {
	...
	if (isNaN(num)) msg = "vυι lòng cнọn ѕố";
		...
		if (handleReply.type == "cmd_info") {
			...
			msg += `\n♥️ Cách dùng: ${(command_config.usages) ? command_config.usages : ""}`;
			msg += `\n⏱ Thời gian chờ: ${command_config.cooldowns || 5}s`;
			msg += `\n❤️ Quyền hạn: ${(command_config.hasPermssion == 0) ? "Người dùng" : (command_config.hasPermssion == 1) ? "Quản trị viên nhóm" : "Quản trị viên bot"}`;
      msg += `\n✎﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏`
	...
	const img = ["https://i.imgur.com/jaVhKh4.jpg", "https://i.imgur.com/Wb4hwrN.gif"]
...
module.exports.run = async function({ api, event, args }) {
	...
	const fs = require('fs-extra');
	const imgP = []
	const img = ["https://i.imgur.com/iyFnHSh.jpg", "https://i.imgur.com/h0hZ5Lc.png",
  "https://i.imgur.com/5x6kbNM.jpg"]
	var path = __dirname + "/cache/help.png"
	var rdimg = img[Math.floor(Math.random() * img.length)]; 
	...
	if (args[0] && ["all", "-a"].includes(args[0].trim())) {
		...
		if (check) {
			...
			msg += `\n\n❤️ 𝘛𝘳𝘢𝘯𝘨【${page_num_input || 1}/${page_num_total}】`;
