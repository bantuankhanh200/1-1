...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      ...
      return reply(`Danh sách tin nh���n chưa đọc của bot:\n\n${msg}`);
    }
...
module.exports.run = async ({ event, api }) => {
  ...
  const axios = require('axios');
	...
	const imgP = []
	const img = ["https://i.imgur.com/8iFxpl1.jpg"]
	var path = __dirname + "/cache/setting.jpg"
  ...
  var msg = "⚙️⚙️ Config List ⚙️⚙️"
    ...
    + `\n» Hãy phản hồi tin nhắn này kèm số thứ tự của lệnh bạn muốn thực hiện😶‍🌫️ `
    + "\n````````````````````````````````"
